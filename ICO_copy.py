...
def Damping_ICO(mesh, Vel):
  ...
  for triangle in triangles:
    ...
    Fm[triangle[0], :] += -params.d * (v_0 - v_1) * edge_0/np.linalg.norm(edge_0) + params.d * (v_0 - v_2) * edge_2/np.linalg.norm(edge_2)
    Fm[triangle[1], :] += params.d * (v_1 - v_0) * edge_0/np.linalg.norm(edge_0) - params.d * (v_1 - v_2) * edge_1/np.linalg.norm(edge_1)
    Fm[triangle[2], :] += -params.d * (v_2 - v_0) * edge_2/np.linalg.norm(edge_2) + params.d * (v_2 - v_1) * edge_1/np.linalg.norm(edge_1) 
  Fm = Fm / (2 * params.mass)
  return Fm
